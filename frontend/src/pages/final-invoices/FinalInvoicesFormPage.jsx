...
function FinalInvoiceFormInner({ invoiceId, onSaved, onCancel }) {
  ...
  useEffect(() => {
    Promise.all([clientService.getAll({ limit: 200 }), productService.getAll({ limit: 200 })])
      .then(([c, p]) => {
  ...
  useEffect(() => {
    ...
    invoiceService.getOne(invoiceId)
      .then(r => {
        ...
        const detectedMode = (inv.items || [])[0]?.igstAmount > 0 ? "inter" : "intra";
        setTaxMode(detectedMode);
        setForm({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit) { await invoiceService.update(invoiceId, payload); toast("Invoice updated"); }
      else        { await invoiceService.create(payload);            toast("Invoice created"); }
      onSaved?.();
    } catch (e) { toast(e.response?.data?.message || e.message, "error"); }
  ...
  return (
    <div style={pageStyle}>
      <PageHeader
        ...
        subtitle={isEdit ? `Editing Invoice ${invoiceId}` : "Create a new tax invoice"}
        actions={
          <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 20, marginBottom: 20 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <Card style={{ padding: "20px 24px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginTop: 14 }}>
              <FS label="Client *" value={form.clientId}
                onChange={e => setField("clientId", e.target.value)}
                options={clientOpts} error={errors.clientId} />
              <Input label="Invoice Date *" type="date" value={form.invoiceDate}
                onChange={e => setField("invoiceDate", e.target.value)} error={errors.invoiceDate} />
              <Input label="Due Date *" type="date" value={form.dueDate}
                onChange={e => setField("dueDate", e.target.value)} error={errors.dueDate} />
              <FS label="Status" value={form.status}
                onChange={e => setField("status", e.target.value)} options={STATUS_OPTS} />
              <div style={{ gridColumn: "1/-1" }}>
                <FS label="Tax Type" value={taxMode}
                  onChange={e => setTaxMode(e.target.value)} options={TAX_MODES} />
              </div>
            </div>
          </Card>
          <Card style={{ padding: "20px 24px" }}>
            <Textarea label="Notes / Remarks" rows={2} value={form.notes}
              onChange={e => setField("notes", e.target.value)} placeholder="Notes for the client…" />
            <div style={{ marginTop: 14 }}>
              <Textarea label="Terms & Conditions" rows={3} value={form.termsAndConditions}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <StatCard label="Grand Total" value={fmt.currency(grandTotal)} icon="💰" color={tokens.success} />
        </div>
      </div>
                ...
                <tr key={item._id} style={{ borderBottom: `1px solid ${tokens.border}20` }}>
                  ...
                  <td style={cs(80)}>
                    <select value={item.gstRate}
                      onChange={e => setItem(idx, "gstRate", parseFloat(e.target.value))} style={sel}>
                  ...
                  <td style={{ padding: "8px 10px" }}>
                    <button onClick={() => removeItem(idx)} disabled={form.items.length === 1}
                      style={{
                        background: "none", border: "none", color: tokens.danger,
        ...
        <div style={{ display: "flex", justifyContent: "flex-end",
          ...
          <div style={{ width: 300 }}>
            ...
            {taxMode === "intra" ? (
              ...
              <TR label="IGST"      value={fmt.currency(igstSum)} />
            )}
...
const inp = {
  ...
  color: tokens.text, borderRadius: 6, padding: "6px 10px",
  fontSize: 13, outline: "none", fontFamily: "inherit",
  width: "100%", boxSizing: "border-box",
};
const sel = { ...inp, cursor: "pointer" };

export default function FinalInvoiceFormPage(props) {
  return <ToastProvider><FinalInvoiceFormInner {...props} /></ToastProvider>;
}